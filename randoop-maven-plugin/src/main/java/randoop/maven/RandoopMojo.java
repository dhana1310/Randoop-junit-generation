...
public class RandoopMojo extends AbstractMojo {
    ...
    private String skipTestCaseGeneration;

    @Parameter(required = true, defaultValue = "500")
    private String testPerFile;

    @Parameter(required = true, defaultValue = "100000")
    private String totalTestCaseLimit;

    @Parameter(required = true, defaultValue = "3000m")
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        try {
            if ("true".equalsIgnoreCase(skipTestCaseGeneration)) {
                return;
            }
            ...
            final String randoopCommandLine = String.join(" ", args);
            getLog().info("Call outside Maven: " + randoopCommandLine);

            if (randoopCommandLine.contains("--testclass=")) {
                runRandoopGenTests(args);
            } else {
                getLog().info("No classes found for test case generation");
            }
        } catch (Exception e) {
//            throw new MojoExecutionException(e.getMessage(), e);
            getLog().error("Error occured generating test-cases : " + e.getMessage());
        }
    }
    ...
    private List<String> buildArgs(final List<URL> urls) {
        ...
        args.add("--output-limit=" + totalTestCaseLimit);
        args.add("--testsperfile=" + testPerFile);
        args.add("--jvm-max-memory=" + jvmMaxMemory);
//    args.add("--debug-checks=true");
        args.add("--junit-package-name=" + packageName);
        args.add("--junit-output-dir=" + targetDirectory);

        skipClasses = skipClasses != null && !skipClasses.isEmpty() ? skipClasses : new ArrayList<>();
        skipPackages = skipPackages != null && !skipPackages.isEmpty() ? skipPackages : new ArrayList<>();
        ...
        for (Class<?> currentClass : allClassesOfPackage) {
            if (!skipClasses.contains(currentClass.getName()) && !skipPackages.contains(currentClass.getPackage().getName())
                    && skipPackages.stream().noneMatch(a -> currentClass.getPackage().getName().startsWith(a))) {
